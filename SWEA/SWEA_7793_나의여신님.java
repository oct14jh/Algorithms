		...
		L:while (true) {
			...
			while(dSize-->0) {
				...
				for (int d = 0; d < 4; d++) {
					nr = dCur[0] + dr[d];
					nc = dCur[1] + dc[d];
					if(nr>=0 && nr<N && nc>=0 && nc<M && map[nr][nc] == '.' ) { // 경계 벗어나지 않고 평범한 지역일때만 감(수연이가 이미 지나간 위치나 자신이 이미 지나온 위치나 돌맹이나 여신위치는 안감)
																				// 수연이가 이미 지나간 위치는 악마가 가봤자 더 늦은 시간에 가므로 의미없음.
			...
			if(sSize == 0) break; // 이동할 수연이의 위치가 없다면 여신의 위치까지 도달할 수 없으므로 끝냄.
			while(sSize-->0) {
				sCur = sQueue.poll();
				for (int d = 0; d < 4; d++) {
					nr = sCur[0] + dr[d];
